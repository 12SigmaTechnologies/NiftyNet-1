...
def cross_entropy(prediction, ground_truth, weight_map=None):
    ...
    #tf.Print(entropy, [entropy], "TT Entropy is : ")
    #tf.Print(entropy[0:3], [entropy[0:3]], "0-3 TT Entropy is : ")
    #print('entropy shape: ', entropy.shape)

    ##add loss for misclassify left and right lung

    alpha_right_loss = 0.5 # coeff of right lung loss
    min_right_lung_seg = 1  # min ind of prob map col of right lung
    max_right_lung_seg = 10  # max ind of prob map col of right lung

    alpha_left_loss = 0.5  # coeff of left lung loss
    min_left_lung_seg = 11
    ...
    tmp_matr = tf.ones([(max_left_lung_seg - min_left_lung_seg) + 1, 1]) #8x1
    tmp_matr = tf.cast(tmp_matr, tf.int32)
    left_lung_mask_expand = tmp_matl*left_lung_mask
    left_lung_mask_expand = tf.transpose(left_lung_mask_expand) #128^3 x 10
    right_lung_mask_expand = tmp_matr *right_lung_mask
    right_lung_mask_expand = tf.transpose(right_lung_mask_expand) # 128^3 x 8

    #----right
    ## penalize predict right lung pixel as left lung pixel: want to compare with GT prob of [0,...,0] 10*1
    # prob map of right lung segments (not include background, background is col0)
    prediction_right = prediction[:, min_right_lung_seg:max_right_lung_seg+1] #128^3 x 10
    left_lung_mask_expand = tf.cast(left_lung_mask_expand, tf.float32)
